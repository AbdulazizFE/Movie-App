...
function UpdateMovie() {
    ...
    { alert(" The id is required!") }
        ...
        (title == 0) { alert(" The title is required!") }
    else if
        (year == 0) { alert(" The year is required!") }
    else if
        (rate == 0) { alert(" The rate is required!") }
    else if
        (storeline == 0) { alert(" The storeline is required!") }
    else if
        (genreid == 0) { alert(" The genre id is required!") }
    else
    {
        fetch('https://localhost:7270/api/Movies/id?Id=' + mo_id,
            {
                method: 'PUT',
                headers: {
...
async function deleteMoviee() {
    ...
    try {
        let response = await fetch(`https://localhost:7270/api/Movies/id?Id=` + m_id, {
  ...
  <form action="" id="Form">
    ...
    <div class="container">
      ...
      <div class="container">
          ...
          <input type="text" id = "rate" placeholder="Rate" step="any" pattern="[-+]?[0-9]*[.,]?[0-9]+"/ required>
      </div>
